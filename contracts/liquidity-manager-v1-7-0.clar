...
(define-public (add-liquidity
    ...
    (begin
        (asserts! (is-eq contract-caller .nexoar-v1-7-0) ERR-UNAUTHORIZED)
        (asserts! (> amount u0) ERR-INVALID-AMOUNT)
        (unwrap!
            (as-contract (contract-call? .vault-tracker-v1-7-0 add-stake provider amount))
            ERR-INVALID-AMOUNT
        )
...
(define-public (remove-liquidity
    ...
    (begin
        ...
        (let ((actual-amount (unwrap!
                (as-contract (contract-call? .vault-tracker-v1-7-0 remove-stake provider
                    amount
                ))
                ERR-INVALID-AMOUNT
            )))
...
(define-public (distribute-pnl (pnl-amount int))
    (begin
        (asserts! (is-eq contract-caller .nexoar-v1-7-0) ERR-UNAUTHORIZED)
        (unwrap!
            (as-contract (contract-call? .vault-tracker-v1-7-0 distribute-pnl pnl-amount))
            ERR-INVALID-AMOUNT
        )
        (if (>= pnl-amount 0)
...
(define-read-only (get-provider-balance (provider principal))
    (let ((info (unwrap-panic (contract-call? .vault-tracker-v1-7-0 get-provider-info provider))))
